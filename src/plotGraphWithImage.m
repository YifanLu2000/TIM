function plotGraphWithImage(I1,I2,X,Y,W,VFCIndex,CorrectIndex,AReverse)
if nargin == 8
    WReverse = AReverse;
end
figure;
...
interval = 20;
Imgwidth = max(siz1(2),siz2(2));
WhiteInterval = 255*zeros(interval,Imgwidth, 3);
if siz1(2) == Imgwidth
    imgtemp = zeros(siz2(1),Imgwidth,3);
    imgtemp(:,1:siz2(2),:) = I2;
    I2 = imgtemp;
else
    imgtemp = zeros(siz1(1),Imgwidth,3);
    imgtemp(:,1:siz1(2),:) = I1;
    I1 = imgtemp;
end
imagesc(cat(1, I1, WhiteInterval, I2));
...
%         l = plot([X(i,1)'; Y(i,1)'], [X(i,2)' ;  Y(i,2)'+size(I1,1)+interval],'linewidth', linwidth, 'color', 'g') ;%'g'
    elseif ismember(i,FalsePos)
        l = plot([X(i,1)'; Y(i,1)'], [X(i,2)' ;  Y(i,2)'+size(I1,1)+interval],'linewidth', linwidth, 'color','r') ;%  [0.8,0.1,0]
    elseif ismember(i,TruePos)
        l = plot([X(i,1)'; Y(i,1)'], [X(i,2)' ;  Y(i,2)'+size(I1,1)+interval],'linewidth', linwidth, 'color','g' ) ;%[0,0.5,0.8]
    end
    l.Color(4) = 0.4;
end
...
axis equal ;
axis([0 Imgwidth 0 siz1(1)+siz2(1)+interval]);
set(gca,'XTick',-2:1:-1)
set(gca,'YTick',-2:1:-1)
%% plot graph
N = size(X,1);
